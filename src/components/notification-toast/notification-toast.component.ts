  ...
  template: `
    <div class="fixed bottom-4 right-4 z-[100] space-y-3">
      ...
      @for (chatToast of chatToasts(); track chatToast.id) {
        <div (click)="openChat(chatToast)" class="max-w-sm w-full bg-white dark:bg-gray-800 shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5 dark:ring-white dark:ring-opacity-10 cursor-pointer">
      ...
      @for (toast of toasts(); track toast.id) {
        <div [class]="getNotificationClasses(toast)"
             class="flex items-center justify-between p-4 rounded-lg shadow-lg min-w-[250px] max-w-sm">
