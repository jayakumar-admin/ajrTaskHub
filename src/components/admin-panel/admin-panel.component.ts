...
<div class="container mx-auto p-4 space-y-8 animate-fade-in">
  ...
  <!-- Section 3: WhatsApp Logs -->
  <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-3xl font-bold text-gray-900 dark:text-white">WhatsApp Logs</h2>
      <button (click)="fetchWhatsAppLogs()" class="px-4 py-2 text-sm bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 rounded-md">Refresh</button>
    </div>
    <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
      Recent outgoing WhatsApp messages.
    </p>
    ...
    @if(logsLoading()) {
      ...
      <div class="bg-red-100 dark:bg-red-900 border border-red-400 text-red-700 dark:text-red-200 px-4 py-3 rounded relative" role="alert">
        <strong class="font-bold">Error!</strong>
        <span class="block sm:inline"> {{ logsError() }}</span>
      </div>
    } @else {
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">To</th>
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            @for (log of whatsAppLogs(); track log.id) {
              <tr>
                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ log.sent_at | date:'short' }}</td>
                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ log.phone_number }}</td>
                <td class="px-4 py-4 whitespace-nowrap text-sm">
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" [class]="log.status === 'success' ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100'">
                    {{ log.status }}
                  </span>
                </td>
                <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-400 max-w-xs truncate">{{ log.message_content }}</td>
                <td class="px-4 py-4 text-xs text-gray-500 dark:text-gray-400 max-w-xs truncate font-mono" [title]="log.error_message || log.meta_message_id">{{ log.error_message || log.meta_message_id }}</td>
              </tr>
            }
            @empty {
              <tr>
                <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No WhatsApp logs found.</td>
              </tr>
            }
          </tbody>
        </table>
  ...
  <!-- Section 4: Global Configuration (WhatsApp) -->
  <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl">
